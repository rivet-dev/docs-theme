...
// ../../node_modules/.pnpm/tm-themes@1.12.2/node_modules/tm-themes/themes/ayu-dark.json
var ayu_dark_default = {
  colors: {
...
// src/lib/textmate-code-theme.ts
var theme = structuredClone(ayu_dark_default);
var replaceColor = "#0b0e14";
var newColor = "#0a0a0a";
...
var textmate_code_theme_default = theme;
  ...
  return (tree) => {
    visit(tree, "element", (node, _nodeIndex, parentNode) => {
      if (node.tagName === "code") {
        ...
        const nodeData = node.data && typeof node.data === "object" ? node.data : null;
        if (nodeData) {
          infoCandidates.push(
            nodeData.meta,
            nodeData.metastring,
        ...
        if (info && typeof info === "string") {
          ...
          const fileVal = fileMatch ? fileMatch[1] ?? fileMatch[2] : void 0;
          const titleMatch = trimmed.match(
            /\btitle=(?:"([^"]*)"|'([^']*)')/
          );
          const titleVal = titleMatch ? titleMatch[1] ?? titleMatch[2] : void 0;
          const sourceFile = fileVal && fileVal.includes("/") ? fileVal : void 0;
          ...
          const resolvedTitle = titleVal ?? fileLabel ?? (titleParts.length > 0 ? titleParts.join(" ") : void 0);
          if (resolvedTitle) {
...
function rehypeShiki() {
  return async (tree) => {
    highlighter ??= await shiki.getSingletonHighlighter({
      themes: [textmate_code_theme_default],
      langs: [
        "bash",
        "batch",
        "cpp",
    ...
    visit(tree, "element", (node, _index, parentNode) => {
      if (node.tagName === "pre" && node.children[0]?.tagName === "code") {
        ...
        const isMermaid = preClassNames.includes("mermaid") || codeClassNames.includes("mermaid") || codeClassNames.includes("language-mermaid") || looksLikeMermaid(textNode.value);
        if (isMermaid) {
          node.properties.language = "mermaid";
          node.properties.className = Array.from(
            /* @__PURE__ */ new Set([...preClassNames, "mermaid"])
          );
        ...
        try {
          const result = highlighter.codeToHtml(textNode.value, {
            lang,
            theme: textmate_code_theme_default.name,
  ...
  return (tree) => {
    ...
    visit(tree, "element", (node) => {
      if (node.tagName === "h2" || node.tagName === "h3") {
        ...
        const parent = node.tagName === "h2" ? headings : headings[headings.length - 1].children;
        parent.push({
    ...
    const isMdx = tree.children.some(
      (n) => typeof n.type === "string" && n.type.startsWith("mdx")
    );
    if (!isMdx) return;
